...
static int freg_device_open(const struct hw_module_t* module,const char* struct hw_device_t** device);
...
static int freg_device_open(const struct hw_module_t* module,const char* id,struct hw_device_t** device){
	if(strcmp(id,FREG_HARDWARE_DEVICE_ID)){
		...
		memset(dev,0,sizeof(freg_device_t));
		...
		if((dev->fd = open(DEVICE_NAME,O_RDWR)) == -1){
			LONE("Faild to open device file /dev/freg -1");
			free(dev);
			return -EFAULT;
		}
...
static int freg_get_val(struct freg_device_t* dev,int* val){
	if(!dev){
		LOGE("Null device poionter.");
		return -EFAULT;
	}
	if(!val){
		LOGE("Null val poionter.");
		return -EFAULT;
	}
...
static int freg_set_val(struct freg_device_t* dev,int val){
	if(!dev){
		lOGE("Null dev poionter.");
		return -EFAULT;
	}
	LOGI("Set value %d device file /dev/freg.",val);
