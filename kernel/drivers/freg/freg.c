...
static int freg_open(struct inode* inode,struct file* filp){
	...
	printk(KERN_ALERT"freg_open fun\n");
...
static int freg_release(struct inode* inode,struct file* filp){
	...
	printk(KERN_ALERT"freg_release fun\n");
	return 0;
}
...
static ssize_t freg_val_store(struct device* dev,struct device_attribute* attr,const char* buf,size_t count){
	struct fake_reg_dev* hdve = (struct fake_reg_dev*)dev_get_drvdata(dev);
	return __freg_set_val(hdve,buf,count);
}
...
static int ___freg_setup_dev(struct fake_reg_dev* dev){
	int err = 0;
	dev_t devno = MKDEV(freg_major,freg_minor);
	memset(dev,0,sizeof(struct fake_reg_dev));

	printk(KERN_ALERT"___freg_setup_dev\n");
...
static int __init freg_init(void){
	...
	err = alloc_chrdev_region(&dev,0,1,FREG_DEVICE_PROC_NAME);
	if(err < 0){
		printk(KERN_ALERT"freg Fialed to alloc char dev reginon.\n");
		goto fail;
	}
	...
	err = ___freg_setup_dev(freg_dev);
	if(err){
		printk(KERN_ALERT"Failed to setup freg device:%d.\n",err);
		goto cleanup;
	}

	freg_class = class_create(THIS_MODULE,FREG_DEVICE_PROC_NAME);
	...
	temp = device_create(freg_class,NULL,dev,NULL,"%s",FREG_DEVICE_PROC_NAME);
	if(IS_ERR(temp)){
		err = PTR_ERR(temp);
		printk(KERN_ALERT"Fialed to create freg device.\n");
		goto destroy_class;
	}

	err = device_create_file(temp,&dev_attr_val);
	if(err < 0){
		printk(KERN_ALERT"Fialed to create attribute val of freg device.\n");
		goto destroy_device;
	}
	...
	freg_create_proc();
	printk(KERN_ALERT"Succedded to initializez freg device.\n");
	return 0;
destroy_device:
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Fake Regiser Driver");
